...
function BranchesPage() {
  ...
  const [editingBranch, setEditingBranch] = useState<{ id: number; name: string; code: string; address?: string | null } | null>(null);
  ...
  const handleEdit = (branch: { id: number; name: string; code: string; address?: string | null }) => {
    setEditingBranch(branch);
    reset({ name: branch.name, code: branch.code, address: branch.address || "" });
    setIsFormOpen(true);
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">{t("settings.branches.title")}</h1>
        ...
        {isFormOpen && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mb-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">
              {editingBranch ? t("settings.branches.editBranch") : t("settings.branches.newBranch")}
            </h2>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    {t("settings.branches.branchCode")} <span className="text-xs text-gray-500">{t("settings.branches.branchCodeNumerical")}</span>
                  </label>
                  <input
                    type="text"
                    {...register("code")}
              ...
              <div className="flex items-center justify-end space-x-4">
                ...
                >
                  {editingBranch ? t("settings.branches.updateBranch") : t("settings.branches.createBranch")}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
          {branchesQuery.isLoading ? (
            <div className="p-8 text-center text-gray-500">{t("settings.branches.loading")}</div>
          ) : branchesQuery.data?.branches.length === 0 ? (
            <div className="p-8 text-center">
              <MapPin className="w-12 h-12 text-gray-400 mx-auto mb-4" />
              <p className="text-gray-500 mb-4">{t("settings.branches.noBranches")}</p>
              <button
                onClick={() => setIsFormOpen(true)}
                  ...
                  {branchesQuery.data?.branches.map((branch) => (
                    <tr key={branch.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-500">
                        {branch.address || <span className="text-gray-400 italic">{t("settings.branches.noAddress")}</span>}
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        ...
                        <button
                          ...
                          disabled={branch.departmentCount > 0 || branch.assetCount > 0}
                        >
